"""
Эндпоинты по аутентификации: регистрация, логин, смена пароля, /me.
Пароли хешируются bcrypt, токены — JWT на 24 часа.
"""
...
from marketlab.api.schemas import UserOut
...
#регистрация: проверяем что email свободен, хешируем пароль, выдаём токен
...
def register(body: RegisterIn, db: Session = Depends(get_db)):
    ...
    user = UserRow(
        email=body.email,
        password_hash=hash_password(body.password),  #в БД летит только хеш, никогда не пароль
    )
    repo.create(user)
    db.commit()
...
#логин: находим юзера по email, сверяем пароль с хешем, выдаём новый токен
@router.post("/login", response_model=TokenOut)
def login(body: LoginIn, db: Session = Depends(get_db)):
    repo = UserRepo(db)
    user = repo.get_by_email(body.email)

    #одно сообщение для обоих случаев(нет такого email / неверный пароль) — чтобы не давать подсказку злоумышленнику
